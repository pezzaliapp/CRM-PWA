...
// Funzioni per la gestione dei prodotti e dei clienti
...
function shouldPopulateInstallazioneFromProduct() {
    return includeInstallazioneCheckboxes() && (installazioneInternoSelected() || installazioneEsternoSelected());
}

function selectProduct() {
    const productList = document.getElementById("productList");
    const selectedIndices = Array.from(productList.selectedOptions).map(opt => parseInt(opt.value, 10));
...
function parseEuropeanFloat(value) {
    ...
    // Rimuove tutti i punti (separatori delle migliaia) e sostituisce la virgola con il punto (decimale)
...
function applyGlobalDiscount() {
    ...
    let globalDiscount = parseFloat(globalDiscountInput.value);
    if (isNaN(globalDiscount) || globalDiscount < 0 || globalDiscount > 100) {
        alert("Inserisci un valore valido per lo sconto globale (0-100).");
        return;
    }

    const selectedRows = Array.from(document.querySelectorAll('#itemsTable tbody tr'))
        .filter(row => row.querySelector('.select-product-checkbox').checked);

    const rowsToUpdate = selectedRows.length > 0 ? selectedRows : Array.from(document.querySelectorAll('#itemsTable tbody tr'));

    if (rowsToUpdate.length === 0) {
        alert("Nessun prodotto disponibile per applicare lo sconto globale.");
        return;
    }

    rowsToUpdate.forEach(row => {
        const discountField = row.querySelector('.article-sconto');
        discountField.value = globalDiscount.toFixed(2);
        calculateTotal(discountField);
    });

    alert(`Sconto globale del ${globalDiscount.toFixed(2)}% applicato.`);
}
...
function useTotal() {
    const totaleInclusoDisplay = document.getElementById('totaleInclusoDisplay');
    if (!totaleInclusoDisplay.classList.contains('hidden')) {
        const totaleInclusoValue = document.getElementById('totaleInclusoValue').textContent;
        const importo = parseEuropeanFloat(totaleInclusoValue);
        if (importo > 0) {
            document.getElementById('importo').value = formatEuro(importo);
        } else {
            alert("Il totale incluso è zero o non valido.");
        }
    } else {
        const totalText = document.getElementById('totalArticlesValue').textContent;
        const totalNumber = parseEuropeanFloat(totalText);
        if (totalNumber > 0) {
            document.getElementById('importo').value = formatEuro(totalNumber);
        } else {
            alert("Il totale articoli è zero o non valido.");
        }
    }
}
...
function loadPotentialClientsFromStorageOrCSV() {
    ...
    if (storedClients && Array.isArray(storedClients)) {
        ...
        nextClientId = potentialClients.length > 0 ? Math.max(...potentialClients.map(c => c.id)) + 1 : 0;
        filterOrderClients();
    } else {
...
function filterOrderClients() {
    ...
    const filtered = potentialClients.filter(c =>
...
document.getElementById("addClientForm").addEventListener("submit", function(event){
    ...
    const newClient = {
        id: nextClientId++,
        name, address, city, province, region, cap, phone, mobile, email, website, clientCode, paymentTerms
    };
...
function calculateTotalTrasporti() {
    ...
    document.querySelectorAll('#itemsTable tbody tr').forEach(row => {
        ...
        const portoWithCharge = portoWithChargeSelected();
        if (includeT && portoWithCharge) total += val;
    });
    return total;
}
...
function toggleMuletto() {
    // Implementa eventuali funzionalità relative al muletto
}
...
// Funzione per aggiornare la nota con il luogo di destinazione
...
function calcola() {
    ...
    if (importo === 0 || isNaN(importo)) {
        alert("Inserisci un importo valido per il calcolo del noleggio!");
        return; 
    }
...
function saveVisitData() {
    ...
    const orderData = {
        ...
        totaleIncluso: document.getElementById("totaleInclusoValue") ? document.getElementById("totaleInclusoValue").textContent : "€0,00",
...
function editVisit(index) {
    ...
    document.getElementById("trasportoForfait").value = visit.trasportoForfait;
    document.getElementById("installazioneForfait").value = visit.installazioneForfait;
...
async function generatePDF() {
    ...
    if (clientDetailsRows.length > 0) {
        ...
        doc.autoTable({
            startY: startY + 60, // Adjusted Y position to account for order note
            head: [["Nome Azienda", "Indirizzo", "Città", "Provincia", "Regione", "CAP", "Telefono", "Cellulare", "Email", "Sito Web", "Cod.Cliente", "Pagamento"]],
            body: tableData,
            styles: { fontSize: 8 },
            columnStyles: {
                0: { halign: 'left' },
    ...
    if (rows.length > 0) {
        ...
        let tableData = Array.from(rows).map(row => {
            ...
            return [
                ...
                cells[3].value,
                cells[4].value,
                ...(includeSconto ? [cells[5]?.value || "0"] : []),
                ...(includePrezzoNetto ? [cells[6]?.value || "0"] : []),
                ...(includeTrasporto ? [cells[7]?.value || "0"] : []),
                ...(includeInstallazione ? [cells[8]?.value || "0"] : []),
                cells[9]?.value || "0"
            ];
        });
    ...
    if (noleggioCheckbox) {
        ...
        doc.autoTable({
            ...
            body: [
                [durata + " mesi", rataMensile, speseContratto, costoGiornaliero, costoOrario]
            ],
            styles: { fontSize: 10 },
            columnStyles: {
                0: { halign: 'left' },
...
// Corretto: parseEuropeanFloat rimuove tutti i punti e sostituisce la virgola con il punto
function parseEuropeanFloat(value) {
    if (!value) return 0;
...
// Aggiornamento dei righi in base alle impostazioni dei prodotti
function updateRowsFromProductSettings() {
    document.querySelectorAll('#itemsTable tbody tr').forEach(row => {
        const code = row.querySelector('.article-code').value.trim();
        const product = products.find(p => p.code === code);
